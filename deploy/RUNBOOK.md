...
## 2026-09-18 검토 후 운영 보완

- 인증서 갱신은 `bash deploy/renew-certificates.sh`로 certbot 성공 후 nginx 문법 검사·reload까지 수행한다. 호스트에서 12시간 주기와 실패 알림을 등록해야 한다.
- 복구는 `bash /backup/restore.sh`를 사용한다. 손상된 gzip/SQL 실패를 성공으로 보고하지 않으며 SQL 오류 시 기존 스키마까지 롤백한다.
- AI가 늦게 응답해도 종료/배정된 상담에 결과를 저장하지 않는다. 단, 메모리 작업 큐는 프로세스 중단 후 자동 재개하지 않으므로 미응답 지표와 번역 실패를 확인한다.
- API는 종료 신호 시 cron을 중단하고 요청/백그라운드 작업을 기다린 뒤 DB 연결을 종료한다. 10초 강제 종료 한계가 있다.
- 로그에서 본문이나 연락처를 찾으려 하지 않는다. 오류 타입/기계 코드/요청 ID로 추적하고, 민감 내용은 권한이 있는 관리자 화면에서 확인한다.
