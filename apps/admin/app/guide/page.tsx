  ...
  return (
    <main style={{ maxWidth: 760, padding: '24px 24px 80px', margin: '0 auto' }}>
      ...
      <Section id="translate" title="5. 번역 미리보기">
        <Note>설정 → 상담·번역 설정에서 외부 API 또는 로컬 Ollama 번역을 선택하고 저장합니다. 모든 운영자가 공유하며 연결 확인은 예제만 사용합니다. 로컬 장애 시 외부로 자동 전환하지 않습니다. AI 응답·요약은 별도 스위치이며 외부 API를 사용합니다.</Note>
        <p>고객이 「담당자 1:1 상담」을 선택한 방은 AI 응답·요약 없이 직접 응대합니다. 「내가 상담하기」로 배정받은 뒤 오른쪽의 「이 상담 상호 번역 사용」을 켜면 일본어 고객 문의를 한국어로 읽고 한국어 답변을 일본어로 번역·수정해서 보낼 수 있습니다. 전체 설정의 자동 번역 허용도 켜져 있어야 합니다.</p>
        <p>
        ...
        <p>대화 표시에서 「원문 + 번역 / 원문만 / 번역 우선」을 선택할 수 있습니다. 번역이 없으면 원문이 보입니다. 번역을 켜면 최근 미번역 고객 메시지 20개도 처리하며, 더 오래된 메시지는 「한국어로 번역」으로 요청합니다. 번역을 꺼도 저장된 번역은 남으며 새 번역과 답변 미리보기를 생략합니다.</p>
      ...
      <Section id="faq" title="10. 자주 묻는 질문">
        <dl style={{ margin: 0 }}>
          {[
            ...
            ['비밀번호를 바꾸고 싶어요.', '비밀번호 변경 화면은 아직 없습니다. 관리자에게 문의해 재설정을 요청하세요.']
          ].map(([q, a]) => (
